...
import swatters.Utils;
import swatters.swing.UserCanceledException;
import sofia.Imager;
import sofia.mops.aladin.AltAzFov;
 ...
 *
 */
public abstract class SOFIAUtils {

	enum Regex {	RAHMS(	"[0-9]{1,2}[h]"+ 
						"[0-9]{1,2}[m]"+ 
						"[0-9]{1,2}[\\.][0-9]{1,}[s]"), 
				DECDMS(	"[+\\-]?" +
						"[0-9]{1,2}[d]" +
	 ...
	 * @param plane
	 */
	static private void addPosToPlane(Astrocoo ac, String name, 
													String plane) {
		// Clean up the name and plane Strings
		name = formatName(name.trim());
		plane = formatName(plane.trim());
		...
		/*
		 *  If catalog plan already exists get it otherwise create it 
		 *  first (after catching the error thrown by trying to get 
		 *  a null plan).
		 */
		try {
	...
	static void setViewSimpleSOFIAROF(ViewSimple viewSimple) {
		...
		double sofiaROF = SOFIAUtils.getViewSimpleSOFIAROF(viewSimple);
		...
		double userInput;
		try {
			userInput = Utils.showNumInputDialog(
											viewSimple.aladin.f,
											0.0, 
											360.0, 
	...
	public static void drawSOFIAROFValue(ViewSimple vs, Graphics g) {
		if (!vs.calque.hasSOFIAImagers()) {
	        ...
	        int stringPosition = (int)(
	        							(vs.rv.getWidth() - stringWidth) / 2.0);    
	...
	/**
	* Opens a new SOFIAExportPosDialog.
	* 
	*/
	public static void openExportPosDialog(Aladin aladin) {
	
		SOFIAExportPosDialog dialog = null;
	
		try {
			dialog = new SOFIAExportPosDialog(aladin);
