...
//   Vector plan;
   ...
   // @SOFIAladin-extension instance variables
   ...
   protected boolean flagAutoDist; // True si l'outil de mesure automatique des distances est activ�
   protected boolean flagSimbad; // True si la r�solution quickSimbad est lanc�
   protected boolean flagVizierSED;  // True si la r�solution SED est lanc�
   protected boolean flagTip;    // True si les tooptips s'affichent sur les sources point�es
//   protected boolean flagSyncView=false; // True si le zoom est synchronis� entre les vues
   ...
   // @SOFIAladin-extension instance variables included
   ...
   /** Retourne le champ de bits qui contr�le les overlays */
   ...
   public boolean hasSOFIAImagers() { return (overlayFlag & SOFIAIMAGERS) == SOFIAIMAGERS; }

   /** Positionnement d'un flag d'overlay - ex: setOverlayFlag("grid",true);
    * @param name le nom de la fonction d'overlay
    * @param flag true pour l'activation, false pour la d�sactivation
    * @return false si probl�me
    */
   ...
   /** Mise � jour des flags d'overlays (les noms s�par�s par une simple virgule).
   ...
   /** Retourne sous la forme d'une chaine de caract�res, tous les overlays actifs 
    * ex : grid,scale,NE */
...
//   protected Source sourceToShow; // source � montrer (thomas, votech)
//   protected Source osourceToShow; // ancienne source � montrer (thomas, votech)
   ...
   /** Ins�re ou enl�ve la scrollbar verticale de la pile si n�cessaire
    * @return true s'il y a eu un changement d'�tat
    */
...
//      return new Dimension(select.getPreferredSize().width- (Aladin.NEWLOOK_V7 && scroll.isShowing()?scroll.getPreferredSize().width:0),200);
   ...
   /** Retourne un plan rep�r� par son indice dans la pile */
...
//      }
   ...
   protected Plan getMyScopeFolder(Plan [] allPlan,Plan p) {
      int nivCherche = p.folder-1;   // Niveau folder � chercher
    ...
    * @param all true tous les plans contenus (r�cursivit�)
...
//            newP[0] = plan[0];
  ...
  /** Retourne le nombre de plans actuellement s�lectionn�s */
  ...
  /** Retourne le nombre de plans actuellement utilises */
   ...
   /** Retourne le nombre d'images (non BG) s�lectionn�es */
   ...
   /** Retourne le nombre de plans Catalog et assimil�s actuellement utilises */
   ...
   /** Retourne le nombre de sources charg�es dans l'ensemble des plans */
   ...
   /** Positionnement du mode du r�ticule 0-sans, 1-normal, 2-large */
...
//   /** Active/D�sactive le background */
...
//   }
   ...
   /** Retourne le premier plan image qui contient la coordonn�e, sinon null */
   ...
   */
   ...
   /** Retourne vrai si la pile est vide � part le plan pass� en param�tre
...
//              System.out.println(plan[i]+" is sync");
    ...
    /** Retourne La liste des noms de plans en cours de traitement (s�par�s par ,) */
...
//       if( !aladin.command.isSync() ) return true;
   ...
   /**  Lib�re le plan pass� en param�tre */
   ...
   */
   ...
   /** Active/D�sactive tous les plans s�lectionn�s */
   protected void setActivatedSet(boolean flagShow) {

      // D�compte du nombre d'images pour �ventuellement passer en mode multiview
      if( flagShow ) {
         int nb=0;
         for( int i=0; i<plan.length; i++ ) {
   ...
   /** Lib�re tous les plans en erreur ou n'ayant aucun objet catalog */
   protected void FreeEmpty() {
      int i,j;

      synchronized( pile ) {

         // Suppression effective (par d�calage)
         for( i=plan.length-1; i>=0; i-- ) {
            if( plan[i].isEmpty() ) {
               if( !plan[i].Free() ) continue;  // Le plan n'est pas lib�rable
               scroll.rm(i);
               Plan ptmp = plan[i];
   ...
   /** Lib�re tous les plans du groupe en cours en les decalant vers le bas */
   protected void FreeSet(boolean verbose) {
      int i,j;

      synchronized( pile ) {
         // Gestion des folders (s�lection de tous les plans qui s'y trouvent)
         for( i=plan.length-1; i>=0; i-- ) {
...
//               plan[1]=ptmp;
   ...
   // Juste pour acc�l�rer un peu
   protected boolean taggedSrc=false;

   /** Retourne true s'il y a au-moins un objet tagu� dans la lsite des plans s�lectionn�s */
  ...
  /** Selection de tous les objets dans les plans selectionnes */
   ...
   /** S�lection de tous les objets des plans activ�s
    * @param mode 0-dans tous les plans, 1-dans les plans s�lectionn�s, 2-les objets tagu�s
    * @param all true => tous les plans, sinon seulement ceux s�lectionn�s
    */
   protected void selectAllObject(int mode) {
      ...
      // TODO : l'envoi d'un tel message doit il se faire automatiquement ? � r�fl�chir
      // envoi d'un message PLASTIC de s�lection des objets
   ...
   /** Enl�ve tous les tags sur les sources */
  ...
  /** Traitement �  faire suite � un changement de frame */
   protected void resumeFrame() {
      ...
      // R�affichage �ventuel du formulaire courant pour convertir
      // la position par d�faut
//      if( aladin.dialog.isVisible() ){
         Server srv = aladin.dialog.server[aladin.dialog.current];

         // on le vide juste avant en cas de formulaire avec tree, sinon impossible � modifier le target
...
//      }
      ...
      // Invalidation des grilles de coordonn�es
      aladin.view.grilAgain();
      aladin.view.repaintAll();

      // N�cessaire dans le cas de plan Healpix
...
//      aladin.view.syncView(1,null,null,true);
   ...
   */
   ...
   protected void setObjet(Obj newobj) {
      if( newobj==oNewobj ) return;   // D�j� ins�r� juste avant
   ...
   */
   ...
   /** Mise � jour des flag des plans:
    *  1) Met � jour les flags ref de tous les plans de la pile
    *  en fonction du nombre de vues qui utilisent ces plans.
    *  Si au-moins une vue utilise un plan particulier, son flag
    *  ref est positionn� � true sinon false
    *  2) On redemande l'activation/desactivation souhait� par l'utilisateur
    */
   ...
   /** Met dans le cache et lib�re le tableau sur les pixels d'origine
    * pour tous les plans images qui ne sont pas visible, ou pour tous les plans
    * images dans le cas o� le mode Pixel courant est 8 bits.
    * @return true s'il y a eu une lib�ration effective, sinon false
    */
...
//      boolean flagFreeAll = aladin.pixel.getPixelMode()==Pixel.LEVEL;
   ...
   * @param p le plan � tester
   */
   protected boolean canBeRef(Plan p) {
      if( p==null ) return false;
      if( p.hasXYorig ) return true;
   	  // ajout thomas : PlanFov peut �tre un plan de r�f�rence
...
//              || p.type==Plan.TOOL && !Projection.isOk(p.projd)) ;
   ...
   */
   ...
   protected boolean setPlanRef(Plan p,int nview) {
      ...
      // s�lectionn�e comme si on avait cliqu� sur le logo du plan
      aladin.view.setSelect(nview);

      // On affecte le plan de r�f�rence � sa vue
      aladin.view.setPlanRef(nview,p);

      // On s�lectionne le plan dans la pile
      selectPlan(p);
      
      return true;
   }
   
   /** Positionne le plan BG comme �tant de r�f�rence. Si la vue que l'on va utilis�e
    * est d�j� un plan BG, centre automatiquement la nouvelle vue sur le m�me champ
    * Si pas possible, garde la position par d�faut
    */
   protected boolean setPlanRefOnSameTarget(PlanBG p) {
      ...
      if( v.isFree() || !Projection.isOk(v.pref.projd) ) return setPlanRef(p,nview);  // pas possible de se mettre � la m�me position
   ...
   */
   ...
   /** S�lection d'un plan particulier */
   ...
   /** Sp�cifie le plan sous la souris */
   ...
   /** Change le nom d'un plan et r�affiche le tout */
   protected void rename(Plan p,String name) {
      p.setLabel(name);
      repaintAll();
   }

   /** Activation du plan p avec �ventuellement scrolling sur
    * la premi�re vue qui le montre.
    * @param p Plan � activer
    * @param flagPaint true si on r�affiche apr�s
    */
...
//      if( p.active ) return;
   ...
   /** D�sactivation de tous les plans BG au-dessus du plan pass� en param�tre qui le cache */
   ...
   private boolean memoClinDoeil=false; // Vrai si on a m�moris� un �tat via le clin d'oeil
   ...
   /** Lorsque l'oeil est cliqu�, il y aura m�morisation de l'�tat d'activation
    * ou non des plans (qui ne sont pas de r�f�rence), puis d�sactivation de ces
    * plans.
    * Si on reclique sur l'oeil, ce sera l'�tat pr�alablement m�moris� qui sera
    * restitu�.
    * Utilise la variable Claque.memoClinDoeil pour savoir si on a d�j�
    * m�moris� un �tat ant�rieur. Et Plan.memoClinDoeil pour m�moriser individuellement
    * chaque �tat de plan.
    */
   ...
   */
   ...
   /** Retourne l'index d'un plan, -1 si non trouv� */
    ...
    * @param mask le pattern de recherche avec jokers � la msdos
    * @param mode 0-avec jokers, 1-simple �galit� stricte des chaines
   ...
   static final int CALIB   = 0x0200;  // calibr�es astrom�triquement
   static final int TRUEIMG = 0x0400;  // Images avec vrais pixels accessibles
   static final int IMG     = 0x0800;  // Tous les types d'images
   static final int SELECTED= 0x0F00;  // Plan s�lectionn�

   /** Retourne true si le param�tre est une masque de bits indiquant un type
    * sp�cial de plan  (c�d > � 0xFF - on n'utilise pas le premier octet � droite) */
   private boolean isSpecialType(int type) { return type>0xFF; }

   /** D�termine le masque de bits en fonction du type de plan et de ses propri�t�s */
   ...
   protected Vector<Plan> getPlans(int type) {
      ...
      for( int i=0; i<plan.length; i++ ) {
         ...
         // S�lection par type "sp�cial" de plan
         if( isSpecialType(type) )  {
            int planSpecialType = getSpecialType(pc);
            if( (type & planSpecialType) == 0 ) continue;

         // S�lection "� l'ancienne" des images (SANS DOUTE A VIRER)
            ...
            // S�lectin par types simple de plan
...
//   }
   ...
   /** Recup�re la liste de tous les plans images */
   protected Vector<Plan> getPlanAllImg() { return getPlans(IMG); }

   /** R�cup�re la liste des plans s�lectionn�s */
   ...
   */
   ...
   /** true si le plan de r�f�rence est un plan en mode Allsky 
   ...
   */
   ...
   /** Retourne la liste des labels des plans en commen�ant par celui du
    * bas de la pile */
   ...
   /** D�coupage du chaine de mesures d'info statistiques sur les pixels
    * afin de pr�parer une entr�e dans le plan "Photometry" */
...
//   }
   ...
   /** Met � jour le plan "Photometry" en fonction des param�tres de iqe */
   ...
   Plan planRotCenter=null; // m�morise le PlanField donc on d�place uniquement le centre de rotation
   ...
   * @param aperture la liste des plans apertures concern�s
...
//         if( plan[i]==planRotCenter ) plan[i].moveRotCenter();
   ...
   * @param Etendue des prorpri�t�s du folder
   ...
   */
   ...
   /** Chargement de la carte de densit� associ�e � un catalogue */
   ...
   * @param label le nom du futur plan , ou null si d�faut
   ...
   */
   ...
   /** Cr�� (ou modifie) un plan resamp
    * @param porig Le plan original (ou d�j� resampl�)
    ...
    * @param keepOrig true si on doit garder l'image d'origine (si non d�j� r�sampl�e)
    */
   protected int newPlanImageResamp(PlanImage porig, PlanImage p,
         String label,int methode,boolean fullPixel,boolean keepOrig) {

      // Faut-il cr�er un clone du plan ou travailler directement sur
      // le plan ?
      PlanImageResamp p2;
   ...
   /** Cr�e un plan MOC en faisant un crop */
   ...
   /** Cr�e un plan MOC en fonction d'un ou plusieurs plans MOCs et d'un op�rateur */
   ...
   /** Cr�e un plan MOC � la r�solution indiqu�e � partir d'une liste d'images et de catalogues. */
   ...
   /** Cr�e une image Algo sur la pile avec l'algo suivant : "p1 fct p2" ou "p1 fct coef" si p2
    * est nul. Si p1 est nul, la premi�re op�rande sera le plan de base lui-m�me et le r�sultat
    * sera affect� au plan de base (pas de cr�ation de plan dans la pile)
    * @param p1 le plan correspondant � la premi�re op�rande ou nul si plan de base
    * @param p2 le plan corresondant � la deuxi�me op�rande
    * @param fct 0:add, 1:sub, 2:mul, 3:div, 4:norm
    * @param coef si p2 est nul, op�rande 2 constante
    * @param conv nom de la convolution ou null si inutilis�
    * @param methode 0:PPV, 1:BILINEAR
    * @return le num�ro du plan dans la pile
    */
   protected int newPlanImageAlgo(String label,PlanImage p1, PlanImage p2,int fct,double coef,String conv,int methode) {
      ...
      // Va cr�er un nouveau plan pour le r�sultat
   ...
   /** Cr�e une image HealpixAlgo sur la pile avec l'algo suivant : "p1 fct p2" ou "p1 fct coef" si p2
    * est nul. Si p1 est nul, la premi�re op�rande sera le plan de base lui-m�me et le r�sultat
    * sera affect� au plan de base (pas de cr�ation de plan dans la pile)
    * @param p1 le plan correspondant � la premi�re op�rande ou nul si plan de base
    * @param p2 le plan corresondant � la deuxi�me op�rande
  ...
  /** Enregistre une image Blink dans le prochain plan libre. */
    ...
    * Remplace la vue pass�e en param�tre si != null
    * */
   ...
   */
    ...
    * @param fmt   le format de l'image (JPEG, FITS...)
    * @param res La resolution de l'image (FULL, PLATE...)
    * @param imgNode noeud image d�crivant l'image � charger
    */
    protected int newPlanImage(URL u,int orig, String label, String objet,
                                     String param,String from,
                                     int fmt,int res,
...
//            inImg = inImg.startRead();
   ...
   private int bestPlace(int n) {
      ...
      if( plan[n].type==Plan.ALLSKYIMG ) {
         ...
         if( i+1==n ) return n; // inutile, c'est moi-m�me
...
//                  && p.projd.agree(pc.projd,null)); i++);
   ...
   /** Duplique le plan courant en ins�rant la copie juste au-dessus
    ...
    *     si m�me classe que le type d'origine
    * @param flagIns Insertion du plan juste au-dessus, sans cr�ation de nouvelle vue
    */
...
//      if( !p.hasAvailablePixels() ) throw new Exception("Not yet supported for this kind of plane");
       ...
       synchronized( pile ) {
          ...
          // Insertion du plan juste au-dessus, sans cr�ation de nouvelle vue
          if( flagIns ) {
          ...
          if( n!=m ) {
             ...
             // VOIR MODIF PF JAN 12 ci-dessous (ligne � supprimer pour revenir � l'�tat ant�rieur)
//             pc.isOldPlan=false;
       ...
       // MODIF PF JAN 12 pour que le nouveau plan deviennent celui de r�f�rence
       if( flagIns ) {
          pc.selected=false;
          pc.active=false;
       } else {
...
//       pc.planReady(true);
   ...
   protected PlanImage greyPlan(PlanImageRGB p) {
      ...
      p2.calculPixelsZoom();
      p2.fmt=PlanImage.JPEG;   // Comme �a on n'inverse pas les pixels � la sauvegarde !!
   ...
   /**
    * Interpr�tation de num�ros d'extensions fits qui auraient �t� indiqu�s
    * en suffixe du nom du fichier suivant la syntaxe suivante
    * [a1,a2,a3-a4...] (la virgule peut �tre remplac� par un ;
    * @param file le nom du fichier
    * @return le tableau contenant les num�ros d'extension ou null si pas d'indication
    * ATTENTION, le tableau n'est pas n�cessairement tri� et il peut y avoir des
    * redondances.
    */
   ...
   /** Cr�ation des plans associ�s � un flux FITSEXTENSION.
...
//      runme.setPriority( Thread.NORM_PRIORITY -1);
   ...
   /**
    * Indique si l'extension n doit �tre retenue. Pour cela, soit numext==null
    * ou n est pr�sent dans le tableau. Si c'est le cas, la valeur est mise � -1 pour pouvoir
    * d�tecter lorsque l'on aura trouv� toutes les extensions souhait�es (voir allFitsExt())
    */
   ...
   /**
    * Retourne true si on a r�cup�r� toutes les extensions fits souhait�es,
    * soit numext==null soit toutes ses valeurs sont � -1
    * @param numext la liste des extensions souhait�es
    * @return true si toutes les extensions ont �t� trouv�es
    */
    ...
    * Cr�e un folder (clignotant le temps du chargement) qui contient
    * tous les �lements.
    * 1) Se fait dans un thread particulier (inhibition du threading interne
    * � PlanImage() et PlanCatalog()) pour s�quentialiser le chargement.
    * 2) Le stream n'est pas ferm� dans PlanImage() et PlanCatalog().
    * 3) Le traitement des erreurs est particulier dans le sens o� une erreur
    * n'emp�che pas le chargement du plan suivant. Il a donc fallu utiliser
    * une ruse pour d�tecter correctement la fin du flux (via le champ
   ...
   private void newFitsExtThread() {
      ...
      try {
         for( int nExt=0; !allFitsExt(numext); nExt++ )  {
            ...
            if( p!=null ) {
               ...
               // Pas terrible: pour d�tecter la fin de fichier on passe par
               // le p.error avec une chaine particuli�re
               ...
               if( keepIt ) {
                  ...
                  // Placement d'un label si ce n'est d�j� fait
      ...
      // Plan actif par d�faut
      p = (Plan)v.elementAt(0);
      if( v.size()==1 ) {
         if( label.charAt(0)=='=' ) label=label.substring(1);
         p.label = label; // On r�cup�re le label du folder
      }
      p.planReady(true);

      // On met tout �a dans la pile
      if( v.size()>1 ) {
         Enumeration e = v.elements();
   ...
   /**
    * Positionnement un flux au d�but d'une extension FITS particuli�re
    * @param in Le flux non encore entam�
    * @param numExt le num�ro d'extension (commence � 0)
    * @throws Exception
    */
   ...
   protected int newPlanImage(String file,MyInputStream inImg,String label,String from,Obj o,ResourceNode imgNode) {
     ...
     * (ne commence pas par =)
     * ou au contraire r�utiliser un plan du m�me nom d�j� existant */
    static protected boolean isNewPlan(String label) {
       return !(label!=null && label.length()>0 && label.charAt(0)=='=');
    }

    /** Cr�e un plan Image sur la pile avec le label indiqu�.
     * Cette m�thode est d�di�e au plugin (voir Aladin.createAladinImage()
     * @param name nom du plan propos�
    ...
    /** Cr�e un plan Catalogue sur la pile avec le label indiqu�.
     * Cette m�thode est d�di�e au plugin (voir Aladin.createAladinCatalog()
     * @param name nom du plan propos�
     * @return nom du plan effectif
     */
...
//    }
    ...
    /** Enregistre une image RGB/FITS dans le prochain plan libre.
     * @param file   Le nom du fichier
     * @param imgNode noeud d�crivant l'image
     */
    ...
    */
     ...
     */
     ...
     // Retourne true si le plan pass� en param�tre peut servir � ajouter des outils draws
     ...
     /** s�lectionne et retourne le plan tool le plus ad�quat, o� le cr�e si n�cessaire */
     protected PlanTool selectPlanTool() { return (PlanTool)selectPlanTool1(false); }
     
     /** s�lectionne et retourne le plan tool ou FoV (APERTURE), o� cr�e un plan tool si n�cessaire */
...
//      pi.setBufPixels8( cube.getFrame(n) );
   ...
   /** Cr�ation d'un plan image � partir des pixels visibles dans la vue pass�e en param�tre */
   ...
   /** Cr�ation d'un nouveau plan catalogue en concatenant toutes les tables des plans
    * pass�s en param�tres, ou � d�faut ceux s�lectionn�s dans la pile.
    * @param pList : liste des plans concern�s ou null pour ceux s�lectionn�s dans la pile
    * @param uniqTable : true si concat�nation dans une unique table homog�ne
    */
   ...
   /** Cr�ation d'un nouveau plan catalogue avec les sources
    *  s�lectionn�es. Je s�lectionne tous les nouveaux objets
   */
   ...
   protected PlanCatalog newPlanCatalogBySources(Vector vSources,String name,boolean uniqTable) {
      ...
      if( !uniqTable ) {
         Vector legs = new Vector(10);      // Juste pour compter le nombre de tables diff�rentes
         ...
         // Fusion en une table unique homog�ne
      } else {

         // G�n�ration d'une l�gende g�n�rique
         ...
         while( e.hasMoreElements() ) {
            ...
            // Cr�ation de la nouvelle ligne de mesures en fonction de la l�gende g�n�rique
   ...
   // Creation d'une chaine de mesures issues d'une source en fonction d'une l�gende g�n�rique particuli�re
   ...
   private String createInfo(Source s,Legende leg) {
      ...
      StringBuffer info = new StringBuffer("<&_X|X>");   // le premier �l�ment ne sert � rien, juste pour compatibilit�
   ...
   */
   ...
   /** Ajoute un plan d�j� pr�par� dans la pile */
   ...
   */
   ...
   /** Subtilit�. Si le nom du plan d�signe commence par =, Aladin
    * doit r�utiliser une case de la pile.
    * Si le plan d�sign� est un num�ro (=@nn), je retourne le label du plan
    * pr�-existant au null si cela correspong � une case vide dans la pile
    * S'il s'av�re que le plan d�sign� n'existe pas =XXXX, je supprime
    * simplement le "=" pour qu'Aladin en cr�e un nouveau
    */
   ...
   // Proc�dure interne de d�coupage d'un plan Catalogue en plusieurs Plan, ou
   // pour chaque table
   ...
   /** D�coupage d'un catalogue : une table par plan */
...
//   }
   ...
   // D�termination du target de d�marrage pour un plan BG
   ...
   // D�termination du radius de d�marrage pour un plan BG
   ...
   /** Cr�ation d'un plan Healpix Multi-Order Coverage Map � partir d'un flux */
   ...
   /** Cr�ation d'un plan Healpix Multi-Order Coverage Map � partir d'un MOC */
...
//   /** Cr�ation d'un plan BG � partir d'un r�pertoire local */
...
//    *
//    * @param path
//    * @param coo coordonn�es de la position centrale de la vue en J2000, ou null si non sp�cifi�e
...
//   }
   ...
   /** Creation d'un plan � partir d'un nom de fichier ou d'une url
    * issu d'une source */
   ...
   /** Creation d'un plan � partir d'un stream ouvert */
   ...
   protected Plan createPlan(InputStream inputStream, String label,String origin) throws Exception {
      ...
      if( n==-2 ) return null;   // De fait aucune cr�ation de plan (cas du FOV)
...
//      bestPlacePost(p);
      ...
      // Affectation du plan aux vues qui utilisaient son pr�d�cesseur
      // dans le cas d'une r�utilisation de plan
...
//System.out.println("de "+s+"/"+plan[s].label+" vers "+t+"/"+plan[t].label+" pour "+n+"x (sens="+sens+")");
    ...
    * et expansion du folder d'arriv�e s'il est collaps�
 ...
 //System.out.println("Permutation "+source.label+" vers "+target.label);
       ...
       synchronized( pile ) {
          ...
          // Si le target �tait un folder collaps�, on le d�collapse
          if( isCollapsed ) {
   ...
   */
   ...
   /** Retourne le premier plan s�lectionn�, ou null si aucun */
   ...
   /** Retourne le premier plan Catalog s�lectionn�, ou null si aucun */
   ...
   /** Retourne le premier plan Image s�lectionn�, ou null si aucun */
    ...
    /** Retourne le premier plan Image simple s�lectionn�, ou null si aucun */
    ...
    */
    ...
    /** Retourne la liste des plans s�lectionn�s */
    ...
    /** S�lectionne tous les plans images simples et en retourne la liste */
    ...
    /** Change pour toute la pile le niveau d'opacit� des images */
    ...
    /** Change pour tous les plans s�lectionn�s le niveau d'opacit� */
    ...
    /** Change pour tous les plans s�lectionn�s le facteur de taille des sources */
    ...
    /** D�termine si le plan doit �tre activ� comme un plan de r�f�rence ou simplement afficher en overlay */
    protected boolean mustBeSetPlanRef(Plan p) {
       boolean setRef=false;
       
       // prochaine vue � utiliser
       ViewSimple v = aladin.view.viewSimple[ aladin.view.getLastNumView(p) ];
       
       // Juste pour du d�buging
       String sDebug=null;
       ...
       // Dans une case vide sans �tre un simple overlay => ref
       ...
       // Dans le cas d'un multiview on priviligiera la cr�ation du plan
    ...
    /** Retourne true si le plan pass� en param�tre peut �tre transparent
     *  V�rifie que la compatibilit� des projections
     */
    ...
    /** V�rifie si un plan peut etre transparent (v�rification uniquement au niveau du type du plan)
     */
  ...
  /** Retourne l'emplacement � utiliser dans la pile. Si on passe un label pr�fix�
   * par le caract�re "=", on cherchera l'emplacement d'un �ventuel plan pr�-existant
   * ayant le m�me label, sinon on utilise un plan libre.
   * Si on passe un nombre, s'assure qu'il y a au-moins autant de plan libre que demand�
   */
    ...
    private int getStackIndex(String label,int nombre) {
       int i;
       // Remplacement d'un plan d�j� utilis� ?
       // Le label doit commencer par '='( ex: =toto). Il peut �galement s'agir d'un num�ro
